...
namespace fc { 
   ...
   namespace raw {
    ...
    template<typename Stream, typename T> inline void unpack( Stream& s, flat_set<T>& value, uint32_t depth = 0 );
    ...
    template<typename Stream, typename K, typename V, typename... A> inline void unpack( Stream& s, flat_map<K,V,A...>& value, uint32_t depth = 0 );
    ...
    template<typename T> inline T unpack( const std::vector<char>& s, uint32_t depth = 0 );
    template<typename T> inline T unpack( const char* d, uint32_t s, uint32_t depth = 0 );
    template<typename T> inline void unpack( const char* d, uint32_t s, T& v, uint32_t depth = 0 );
} }
